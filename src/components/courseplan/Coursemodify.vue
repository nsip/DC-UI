        ...
        <div class="row courseplan">
            <transition name="slide-fade">
                <div v-if="!isShow" class="col-3" style="padding-right:10px">
                    <app-couser-reminder :coursename="lesson.thecourse" :selectedarea="lesson.thearea" :selectedcourse="lesson.thesubject" :selectedstage="lesson.thestage" />
                </div>
            </transition>
            <div :class="isShow?'col-9':'col-6'">
                <!-- <div class="editor" v-for="(l, index) in lessons" :key="index" :style="l.isDisplay?'display:block':'display:none'"> -->
                <div class="editor" :key="index" :style="selected_lesson.isDisplay?'display:block':'display:none'">
                    <q-editor toolbar-text-color="white" toolbar-outline toolbar-bg="deep-purple-4" min-height="40rem" v-model="selected_lesson.editcontent" :toolbar="[
                    ...
                    <q-list class="bottom-tips">
                        <q-item>
                            <q-item-side>
                                <!-- <p>Title: {{ l.summary }}</p> -->
                                <p>Title: {{ selected_lesson.summary }}</p>
                            </q-item-side>
                            <q-item-main style="text-align:center" text-color="dark">
                                <p style="font-size:14px;"><i class="far fa-smile" style="margin-right:10px;" />Please remeber to click save, when you make changes.</p>
                            </q-item-main>
                            <q-item-side right>
                                <p>Lesson No.{{ selected_index + 1 }}</p>
                            </q-item-side>
                        </q-item>
                    </q-list>
            ...
            <div class="col-3" style="padding-left:10px;">
                <q-chips-input inverted color="deep-purple-4" chips-color="dark" chips-bg-color="white" v-model="keywords" float-label="Enter the key words" class="search-input" @keyup="search">
                </q-chips-input>
                <div class="search">
            ...
            <q-card class="col-9 l list-of-lesson">
                <q-card-title class="relative-position q-ma-sm">
                    ...
                    <q-btn class="absolute" icon="fas fa-plus" outline rounded flat color="deep-purple-9" style="right: 8px;" @click="add" />
                    <q-card-separator style="margin-top: 15px; margin-bottom: 15px" />
                    <span slot="subtitle">
                        <img src="../../assets/list.png" class="lesson-sequence-logo">
                        <q-field :label-width="2" label="Lesson Sequence">
                            <q-input v-model="lesson.thedescription" color="deep-purple-9" />
                        </q-field>
                    </span>
                </q-card-title>
                <q-item-separator />
                        ...
                        <div class="col-lg-2 col-md-4 col-sm-6 col-xs-12" v-for="(l, index) in lessons" :key="index">
                            <q-list class="q-ma-xs addLesson">
                                <q-item>
                                    ...
                                    <q-item-side right>
                                        <q-btn class="q-ma-xs" icon="fas fa-trash-alt" size="8px" outline round @click='removeLesson(index)' />
                                    </q-item-side>
                                </q-item>
                                <q-item>
                                    <q-btn outline style="color: #4527a0; width:100%" rounded @click='selectedLesson(index)'>Lesson {{ index + 1 }}</q-btn>
                                </q-item>
                                <q-item>
                                    <div>
                                        <q-input class="q-ma-sm" color="deep-purple-5" placeholder="Lesson title" v-model="l.summary" />
                                    </div>
                                </q-item>
                            </q-list>
                        </div>
                ...
                <q-item-separator />
                <q-card-title class="relative-position q-ma-sm">
                    <b style="color:gray">Learning Area: {{lesson.thearea}} | Subject: {{lesson.thesubject}} | Stage: {{lesson.thestage}}</b>
                    <div slot="right" class="row items-center">
                        <q-btn flat outline color="deep-purple-5" label="Submit All Lessons" @click="submit">
                        </q-btn>
                    </div>
                </q-card-title>
            </q-card>
            <q-card class="col-3 l list-of-link">
                ...
                <q-card-main class="link-list q-ma-xs" style="color:gray">
                    <q-item v-for="(link,index) in seletedurl" :key="index">
                        <q-item-side>
                            <q-btn flat round icon="fas fa-minus-circle" outline color="deep-purple-5" @click="deletelink(index)" />
                        </q-item-side>
...
export default {
    ...
    created() {
        ...
        this.lessons[0].isDisplay = true
        console.log(this.lesson)
        this.selected_lesson = this.lessons[0]
    ...
    methods: {
        ...
        selectedLesson(selectedId) {
            ...
            console.log("selecetd-index: ", selectedId)
            ...
            this.selected_lesson = this.lessons[selectedId]
            this.selected_index = selectedId
